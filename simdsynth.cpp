...
// Hard-coded wavetables
static float sineTable[WAVETABLE_SIZE];
static float sawTable[WAVETABLE_SIZE];
...
struct Voice {
    ...
    float phase;             // Main oscillator phase (0 to 1 for wavetable)
    ...
    float subTrack;          // Sub-oscillator pitch tracking (0 to 1)
    int wavetableType;       // Wavetable type: 0=sine, 1=sawtooth
};
...
// SIMD-based wavetable lookup with linear interpolation
inline SIMD_TYPE wavetable_lookup_ps(SIMD_TYPE phase, const float* table) {
    ...
    // Load samples for indices and indices+1
    ...
    // Linear interpolation: value1 + frac * (value2 - value1)
...
// Generate samples with wavetable or sine oscillator for main, sine for sub-oscillator
void generateSineSamples(Voice *voices, int numSamples, Filter &filter,
    ...
    for (int i = 0; i < numSamples; i++) {
            ...
            if (currentTime != chords[currentChord].startTime) {
                ...
                for (size_t v = 0; v < MAX_VOICE_POLYPHONY; v++) {
                    ...
                    if (voices[v].active) {
                        ...
                        voices[v].lfoRate = 0;
                        voices[v].lfoDepth = 0;
                        voices[v].subTune = -12.0f;
                        voices[v].subMix = 0.5f;
                        voices[v].subTrack = 1.0f;
        ...
        for (int group = 0; group < (MAX_VOICE_POLYPHONY + 3) / 4; group++) {
            ...
            // Compute main oscillator output based on wavetable type
            ...
            // Compute sub-oscillator output (remains sine)
...
int main(int argc, char* argv[]) {
    ...
    int wavetableType = 0; // Default to sine wavetable
    ...
    for (int i = 0; i < MAX_VOICE_POLYPHONY; i++) {
        ...
        voices[i].subFrequency = 0.0f;
        voices[i].subPhase = 0.0f;
        voices[i].subPhaseIncrement = 0.0f;
        voices[i].subTune = -12.0f;
        voices[i].subMix = 0.5f;
        voices[i].subTrack = 1.0f;
        voices[i].wavetableType = wavetableType;
        for (int j = 0; j < 4; j++) {
            voices[i].filterStates[j] = 0.0f;
        }
    }
